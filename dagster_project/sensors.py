...
def create_prediction_job_sensor(context):
    ...
    calendar = pd.read_csv("dagster_project/data_storage/calender.csv")
    ...
    time_modified = (utc_dt + timedelta(hours=1)).replace(tzinfo=pytz.utc)
    ...
    if closest_race['EventFormat'] == 'conventional':
        ...
        if session_time < time_modified:
            ...
            return RunRequest(
                run_config={'ops': {'get_session_data': {"config": {'event_type': closest_race['EventFormat'],
